...
| `outbox` | ✅ Foundation (Slice 0) + articles Slice 1 (#357) | Transactional outbox table + drainer worker + `WebhookEventDispatcher` env-gated routing (ADR-0026 #356). `OUTBOX_DRAINER_ENABLED=False` (default) → legacy direct fan-out; `=True` → single insert в outbox + drainer fan-out per subscriber. **Slice 1 (#357)**: POST /articles handler refactored — `ArticleRepository.create_atomic` (no inner commit) + audit + outbox.enqueue (если enabled) → single `session.commit()` → atomic per ФЗ-152 §22 invariant. Rollback semantics tested (audit failure → no commit → article rollback). Slice 2+ переводит chat/collaborators/vault repos на тот же pattern. |
