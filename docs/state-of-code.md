...
| ФЗ-152 §21 — physical right-to-forget (chat) | ✅ DONE (#341) | `ChatCleanupWorker` (daily poll, env-gated) physically deletes soft-deleted chat_sessions past retention (30d default, configurable) + expired sessions never-deleted; CASCADE на chat_messages. Reuses pd_overdue_worker pattern. |
