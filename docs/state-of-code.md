...
| `outbox` | ✅ Slices 0-4a (#356 #357 #358 #328) | Transactional outbox table + drainer worker + `WebhookEventDispatcher` env-gated routing (ADR-0026 #356). `OUTBOX_DRAINER_ENABLED=False` (default) → legacy direct fan-out; `=True` → single insert в outbox + drainer fan-out per subscriber. **Slice 1 (#357)**: POST /articles handler — atomic via `create_atomic` + single `session.commit()`. **Slice 2 (#358)**: POST /chat/sessions/{id}/escalate (via `create_escalation_atomic`) + POST /collaborators/{id}/reviews. **Slice 3 (#328)**: оставшиеся 9 collaborator lifecycle endpoints — POST/DELETE/activate/suspend/onboarding/portal-access в `router.py` + create/cancel/accept/complete/fail в `service_orders_router.py` (webhook dispatch перенесён ДО `session.commit()`). Vault emergency unlock (ADR-0021) уже был atomic — out of slice scope. **Slice 4a**: `OutboxCleanupWorker` (mirror WebhookCleanupWorker pattern) — physical-delete flushed outbox rows past retention (default 30 days per ADR-0026 open-question #4). Env-gated: `OUTBOX_CLEANUP_WORKER_ENABLED`/`OUTBOX_CLEANUP_RETENTION_DAYS`/`OUTBOX_CLEANUP_POLL_INTERVAL_SECONDS`. Rollback semantics covered тестами (audit / dispatch failure → no commit → business row rolled back). Slice 4b — remove legacy direct-dispatch path in `WebhookEventDispatcher` (forces `OUTBOX_DRAINER_ENABLED=True`, deployment-impacting). |
