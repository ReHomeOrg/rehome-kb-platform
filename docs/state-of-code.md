...
| `outbox` | ✅ Slices 0-2 (#356 #357 #358) | Transactional outbox table + drainer worker + `WebhookEventDispatcher` env-gated routing (ADR-0026 #356). `OUTBOX_DRAINER_ENABLED=False` (default) → legacy direct fan-out; `=True` → single insert в outbox + drainer fan-out per subscriber. **Slice 1 (#357)**: POST /articles handler — atomic via `create_atomic` + single `session.commit()`. **Slice 2 (#358)**: POST /chat/sessions/{id}/escalate (via `create_escalation_atomic`) + POST /collaborators/{id}/reviews (webhook dispatch перенесён ДО `session.commit()`) — atomic per ФЗ-152 §22 invariant. Rollback semantics covered тестами (audit failure → no commit → business row rolled back). Slice 3+ (vault security-sensitive) + Slice 4 (cleanup worker + legacy path removal). |
