...
|---|---|---|
| `articles` | ✅ MVP | CRUD + версионирование + Postgres FTS поиск + tags (case-insensitive #346) + categories + frontend admin форма + Pydantic Literal enums для audience/status/language (#353) + **Q&A module** (2026-05-28, ТЗ §2): user submits → staff moderates → public ANSWERED display. Backend `POST/GET /articles/{slug}/questions` + admin `GET/POST /admin/article-questions[/answer\|/dismiss]`. Frontend: `ArticleQaSection` под статьёй + `/admin/article-questions` moderation queue. ФЗ-152: body не в audit metadata, public list скрывает author_sub. |
...
| `outbox` | ✅ Slices 0-4 (#356 #357 #358 #328 #330) | Transactional outbox table + drainer worker (ADR-0026 #356). `WebhookEventDispatcher` всегда пишет в outbox (Slice 4b: legacy direct-dispatch путь удалён); drainer fan-out'ит rows на subscribers. **Slice 1 (#357)**: POST /articles handler — atomic via `create_atomic` + single `session.commit()`. **Slice 2 (#358)**: POST /chat/sessions/{id}/escalate (via `create_escalation_atomic`) + POST /collaborators/{id}/reviews. **Slice 3 (#328)**: оставшиеся 9 collaborator lifecycle endpoints — POST/DELETE/activate/suspend/onboarding/portal-access в `router.py` + create/cancel/accept/complete/fail в `service_orders_router.py` (webhook dispatch перенесён ДО `session.commit()`). Vault emergency unlock (ADR-0021) уже был atomic — out of slice scope. **Slice 4a (#330)**: `OutboxCleanupWorker` — physical-delete flushed rows past retention (default 30 days). **Slice 4b**: dispatcher conditional удалён (always-outbox); `OUTBOX_DRAINER_ENABLED` default flipped to True; drainer env-gate сохраняется для split-pod deployments. Rollback semantics covered тестами (audit / dispatch failure → no commit → business row rolled back). |
...
| Audit-log export форматы | ✅ DONE (CSV #172 + JSONL #352) | `GET /audit-log/export.csv` (Excel-friendly, UTF-8 BOM) + `GET /audit-log/export.jsonl` (newline-delimited JSON, programmatic ingest). Hard cap 10k rows. Admin task export wires `format=json` → .jsonl, `format=csv` → .csv (см. `_build_export_url`). |
...
2. ~~Vault Stage 2 emergency access~~ ✅ ADR-0021 принят (Вариант A,
   2026-05-21). Backend foundation #333 + owner setup ceremony UI #334
   + admin emergency-unlock UI #335. Full backend + frontend stack:
   Shamir SSS over GF(256) (split/combine с HMAC integrity), client-
   side AES-GCM(KEK, escrow_key) wrap, severity-by-reason mapping + РКН
   policy (incident only), audit + security_incident wiring, cross-impl
