...
# 100%. Паттерн ловит ТОЛЬКО завершающую фразу-приглашение в поддержку/оператора
# в самом конце ответа (не трогает упоминания в середине — якорь `$`, и не
# пересекает границы предложений `[.!?\n]`). Применяется в router лишь когда
# ответ содержательный (`has_usable_context`), чтобы не срезать уместную
# эскалацию при реальном отсутствии ответа в базе.
_OPERATOR_FOOTER_RE = re.compile(
    r"(?:(?<=[.!?\n])|^)[^.!?\n]*"
    r"(?:операт\w*\s+поддержк\w*|обрат\w+\s+(?:в\s+поддержк\w*|к\s+операт\w*)"
    r"|сообщ\w+\s+операт\w*|перейд\w+\s+в\s+раздел\s+поддержк\w*)"
    r"[^.!?\n]*[.!?]*\s*$",
    re.IGNORECASE,
)
...
def strip_operator_footer(text: str) -> str:
    ...
    """
    return _OPERATOR_FOOTER_RE.sub("", text).rstrip()
