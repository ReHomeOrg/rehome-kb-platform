...
# #388: малая LLM (yandexgpt-lite) изредка дописывает дежурную приписку об
# операторе даже когда ответила по существу — overlay-промпт давит её, но не на
# 100%. Паттерн ловит ТОЛЬКО завершающую фразу-приглашение (якорь `$`), причём
# приглашение должно начинаться сразу после границы предложения (или начала
# текста) — допускаются лишь filler-опенеры («если у вас остались вопросы,»,
# «пожалуйста,», «при необходимости,»). Это КРИТИЧНО: без такого ограничения
# ведущий `[^.!?\n]*` пересекал бы запятые и съедал содержательный текст,
# приклеенный к приписке через запятую (напр. «Ремонт покрыт страховкой,
# обратитесь в поддержку» → потеря содержательной части). Применяется в router
# лишь при содержательном ответе (`has_usable_context`), чтобы не срезать
# уместную эскалацию при реальном отсутствии ответа в базе.
_OPERATOR_FOOTER_RE = re.compile(
    r"(?:(?<=[.!?\n])|^)\s*"
    # optional filler-опенеры (ТОЛЬКО они допускаются перед приглашением)
    r"(?:если\s+(?:у\s+вас\s+)?(?:остал\w+|есть|возникн\w+|появ\w+)[^.!?\n]*?[,—-]\s*)?"
    r"(?:пожалуйста[,\s]+)?"
    r"(?:(?:по\s+(?:любым|всем)\s+вопросам|при\s+необходимости)[,\s]+)?"
    # само приглашение к оператору / в поддержку
    r"(?:обрат\w+\s+(?:в\s+поддержк\w*|к\s+операт\w*)"
    r"|сообщ\w+\s+операт\w*"
    r"|перейд\w+\s+в\s+раздел\s+поддержк\w*"
    r"|операт\w*\s+поддержк\w*)"
    r"[^.!?\n]*[.!?]*\s*$",
    re.IGNORECASE,
)
...
def strip_operator_footer(text: str) -> str:
    """Срезать завершающую фразу-приглашение к оператору/в поддержку (#388).

    Detterministic-нетто поверх overlay-промпта «не зови оператора»: малая LLM
    изредка всё же дописывает дежурную приписку. Срезает ТОЛЬКО завершающую
    фразу (якорь на конец текста, не пересекает границы предложений), поэтому
    упоминания «обратитесь в поддержку» в середине ответа не трогаются.

    ВАЖНО: вызывать только когда ответ содержательный (`has_usable_context`) —
    при реальной эскалации (нет ответа в базе / конфликт) приписка уместна и
    должна остаться. Гейт — в router.

    Guard: если стрип оставил бы пустую строку (edge — весь ответ оказался
    приглашением) — возвращаем исходный текст. Лучше оставить приписку, чем
    потерять ответ.
    """
    stripped = _OPERATOR_FOOTER_RE.sub("", text).rstrip()
