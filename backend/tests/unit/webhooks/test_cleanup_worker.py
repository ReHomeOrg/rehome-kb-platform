"""Unit tests для WebhookCleanupWorker (#342, ФЗ-152 §21)."""
...
def _settings(interval: float = 0.05, retention_days: int = 30) -> Any:
    s = MagicMock()
    s.webhook_cleanup_poll_interval_seconds = interval
    s.webhook_cleanup_retention_days = retention_days
    return s
...
def _patch_repo(monkeypatch: pytest.MonkeyPatch, *, delete_count: int) -> MagicMock:
    ...
    repo_instance.hard_delete_soft_deleted = AsyncMock(return_value=delete_count)
    repo_cls.return_value = repo_instance
    monkeypatch.setattr("src.api.webhooks.cleanup_worker.WebhookRepository", repo_cls)
...
async def test_run_once_zero(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    worker = WebhookCleanupWorker(session_factory=factory, settings=_settings())
    assert await worker._run_once() == 0
    repo.hard_delete_soft_deleted.assert_awaited_once()
    assert repo.hard_delete_soft_deleted.call_args.kwargs["retention"].days == 30


@pytest.mark.asyncio
async def test_run_once_commits_on_delete(monkeypatch: pytest.MonkeyPatch) -> None:
    factory, sessions = _factory()
    _patch_repo(monkeypatch, delete_count=4)
    worker = WebhookCleanupWorker(session_factory=factory, settings=_settings())
    assert await worker._run_once() == 4
...
async def test_lifecycle(monkeypatch: pytest.MonkeyPatch) -> None:
    factory, _ = _factory()
    repo = _patch_repo(monkeypatch, delete_count=0)
    worker = WebhookCleanupWorker(session_factory=factory, settings=_settings(interval=0.05))
    worker.start()
    await asyncio.sleep(0.1)
    await worker.stop()
    assert repo.hard_delete_soft_deleted.await_count >= 1
...
async def test_loop_survives_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    repo_cls = MagicMock()
    repo_instance = MagicMock()
    repo_instance.hard_delete_soft_deleted = AsyncMock(side_effect=[RuntimeError("boom"), 0])
    repo_cls.return_value = repo_instance
    monkeypatch.setattr("src.api.webhooks.cleanup_worker.WebhookRepository", repo_cls)
    worker = WebhookCleanupWorker(session_factory=factory, settings=_settings(interval=0.05))
    worker.start()
    ...
    await worker.stop()
    assert repo_instance.hard_delete_soft_deleted.await_count >= 2
